...
int print_usage(int argc, char **argv)
{
	...
	fprintf(stderr,
			...
			"    -w <warn_treshold>       Return a warning if more than <warn_treshold>\n"
			"                             updates are available\n"
			"    -c <crit_treshold>       Return a critical error if more than\n"
			"                             <crit_treshold> updates are available\n"
			"    -security-update         Apply security updates instead of showing them\n"
			"    -update                  Apply all updates instead of showing them\n"
...
#undef OPT_NONE
	...
	if (get_opt_defined(opts, OPT_LOCK))
	{
		...
		if (result < 0)
		{
			...
			status = 1;
			printf("UPDATE Warning | Failed to acquire the lock file\n");
			goto fail_lock_lock;
		}
