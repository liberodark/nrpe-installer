...
#undef OPT_NONE
	...
	LOG("pk_client_refresh_cache >>\n");
   	 pk_results = pk_client_refresh_cache(cli, TRUE, ctx.cancellable, progress_cb, &ctx, &gerror);
    	if (!pk_results)
    	{
        const char *gerror_msg = (gerror && gerror->message) ? gerror->message : "unknown error";
        LOG("pk_client_refresh_cache failed : %s\n", gerror_msg);
	...
	{
		const char *gerror_msg = (gerror && gerror->message) ? gerror->message : "unknown error";
        LOG("pk_client_get_updates failed : %s\n", gerror_msg);
	...
	LOG("pk_results_get_package_array >>\n");
	pkgs = pk_results_get_package_array(pk_results);
	g_object_unref(pk_results);
	if (!pkgs)
	{
		LOG("pk_results_get_package_array failed !!\n");
	...
	LOG("pk_results_get_update_detail_array >>\n");	
	upds = pk_results_get_update_detail_array(pk_results);
	g_object_unref(pk_results);
	if (!upds)
	{
		LOG("pk_results_get_update_detail_array failed\n");	
